    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl p-20 text-center border border-gray-100">
          ...
          <h3 className="text-lg font-normal text-black tracking-tight mb-1">No appointments yet</h3>
          <p className="text-gray-400 text-sm font-normal">New bookings will appear here automatically.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {appointments.map((appt) => (
            <div key={appt._id} className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-md transition-all duration-300">
              <div className="p-6 pb-0 flex justify-between items-start">
                <div className="w-12 h-12 bg-primary-light rounded-2xl flex items-center justify-center text-primary">
                  <User className="w-6 h-6" />
                </div>
                <span className={`px-3 py-1 rounded-full text-[10px] font-normal uppercase tracking-wider ${
                  appt.status === 'Pending' ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-primary'
                }`}>
                  {appt.status}
                </span>
              </div>
              ...
              <div className="p-6 space-y-4 flex-1">
                <div>
                  <h4 className="text-lg font-normal text-black tracking-tight">{appt.patientName}</h4>
                  <p className="text-primary text-xs font-normal mt-0.5">{appt.category} • {appt.doctor}</p>
                </div>

                <div className="space-y-2.5 pt-2">
                  <div className="flex items-center gap-3 text-xs text-gray-500 font-normal">
                    <Calendar className="w-4 h-4 text-gray-400" /> {appt.date || 'Not specified'}
                  </div>
                  <div className="flex items-center gap-3 text-xs text-gray-500 font-normal">
                    <Clock className="w-4 h-4 text-gray-400" /> {appt.time || 'Not specified'}
                  </div>
                  <div className="flex items-center gap-3 text-xs text-gray-500 font-normal">
